...
export class CartService {
  ...
  removeFromCart(item: CartItem): void {
    ...
    this._snackBar.open('1 item removed from cart', 'Close', { duration: 3000 })
  ...
  removeRow(item: CartItem, notifyUser: boolean = true): Array<CartItem> {
    const filteredItems = this.cart.value.items.filter((_item) => _item.id !== item.id)
    
    if (notifyUser){
      this.cart.next({items: filteredItems})
      this._snackBar.open('Whole row of items removed from cart', 'Close', { duration: 3000 })
    }
